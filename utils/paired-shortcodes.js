module.exports = {
	...
	japanese: function (content, speaker) {
		let speakerHtml = speaker ? `<div class="text-lg text-center mb-0"><span class="bg-white text-black px-2 py-1 border-solid border-2 shadow-lg rounded-t-3xl">${speaker}</span></div>` : "";

		return `${speakerHtml}<div class="japanese not-prose text-3xl m-4 p-4 border-solid border-2 shadow-lg rounded-lg text-white" style="background-color: darkslategray;">${content}</div>`;
	},
}
